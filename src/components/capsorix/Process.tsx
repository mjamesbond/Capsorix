...
const ICONS = [Lightbulb, Compass, Palette, Code2, Rocket, LifeBuoy] as const;
    ...
    <section id="process" className="relative section overflow-hidden">
      ...
      <div className="container relative">
        <Reveal className="text-center max-w-3xl mx-auto mb-24">
          <p className="text-xs font-medium tracking-[0.35em] uppercase text-primary mb-5">{t.process.kicker}</p>
          <h2 className="font-display text-5xl md:text-7xl font-medium leading-[1.05]">
            {t.process.titleA}
            <span className="text-gradient-gold italic">{t.process.titleB}</span>
          </h2>
          <p className="mt-6 text-lg text-muted-foreground leading-relaxed">{t.process.lead}</p>
        </Reveal>
            ...
            {steps.map((step, i) => (
              ...
              >
                <div className="relative mb-6">
                  ...
                  <span className="absolute -top-2 -right-2 w-7 h-7 rounded-full bg-gold-animated text-primary-foreground text-xs font-bold flex items-center justify-center shadow-gold" dir="ltr">
                    {i + 1}
                  </span>
                </div>
