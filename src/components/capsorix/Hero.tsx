...
const CodeRain = lazy(() => import("./CodeRain"));

const Hero = () => {
  const { t } = useI18n();
  const { allowDecorativeMotion, allowHeroCodePanel } = usePerformanceProfile();
  ...
  return (
    <section className="hero-stage relative min-h-screen flex items-center overflow-hidden pt-32 pb-24">
      {/* LCP image remains immediate; motion layers are capability-gated. */}
      <div ref={bgRef} className="hero-media absolute inset-0 -z-10 will-parallax scale-110">
        <img
          src={heroBg}
      ...
      {allowDecorativeMotion && (
        <>
          <div
            ref={orbARef}
            aria-hidden
            className="hero-orb absolute top-[18%] -left-32 w-[32rem] h-[32rem] rounded-full bg-primary/10 blur-[160px] will-parallax"
          />
          <div
            ref={orbBRef}
            aria-hidden
            className="hero-orb absolute bottom-[12%] -right-32 w-[36rem] h-[36rem] rounded-full bg-primary-glow/[0.06] blur-[180px] will-parallax"
          />
        </>
      )}
      ...
      <div className="container relative">
        <div
          className={`grid gap-16 items-center ${
            allowHeroCodePanel ? "lg:grid-cols-[1.1fr_0.9fr]" : "lg:grid-cols-1"
          }`}
        >
          <div
            className={`hero-copy max-w-3xl text-center lg:text-start ${
              allowHeroCodePanel ? "" : "lg:max-w-4xl"
            }`}
          >
          ...
          {allowHeroCodePanel && (
            <div
              className="hero-code-shell hidden lg:block animate-fade-in-up"
              style={{ animationDelay: "0.35s" }}
            >
              <Suspense fallback={<div aria-hidden className="hero-code-placeholder rounded-2xl" />}>
                <CodePanel />
              </Suspense>
            </div>
          )}
